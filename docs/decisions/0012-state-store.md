...
The state itself is tiny and fixed: 4,320 arms × 4 float64 = **135 KiB per shard**. (The ticket sketches the arm as BIN-class × currency × amount-band × processor; ADR-0006 R39 shipped (bin class × region × sca × mandate × band) × processor = 4,320 arms, and every byte figure here is R39's space. The store does not care which key is in force — `arm_schema_hash` in the `boot` row names it, and R40's re-bucketing is what changes it, at `[W7]`(d)'s price.) (`[W8]`: 0.14 MB of learned state at *either* pace, against 177 GB/day of trace). That asymmetry — fixed-size state, unbounded history — is why the ticket's "in-memory + SQLite checkpoint" option is half right: the arrays *should* be in memory, and the *log* is what needs a backend.
...
- **DuckDB as the query layer** — the ticket's own question — is a yes, on the export side only: it is a Python/`analysis/` reader of sealed columnar artifacts, never an engine dependency (ADR-0001's `CGO_ENABLED=0` and ADR-0011's import DAG both keep it out of `internal/`). `[W5]`(c) is why that placement matters rather than being a detail: the columnar read itself runs at 21.65 M rows/s while the same rows aggregated in CPython run at 4.04 M, so the format's win and the interpreter's loss are separate quantities, and a vectorized engine keeps the first and drops the second. This spike cannot measure DuckDB and does not claim to.
...
- **The rollback journal is disqualified at the same durability**: 1,068 rows/s against WAL's 7,015 one-row-per-commit, 6.6× worse, because it fsyncs twice per commit. It reaches 162,413 rows/s only at batch 1024, i.e. only by becoming a batch store with a worse tail.
...
7. **Retention volume.** If 205 GB/day per fleet (82 TB across the window) exceeds the storage budget, reopen the *window* (ADR-0004's 400 days) or the row width (§4.4's ladder) — in that order, because the row width is what makes the queries answerable and Appendix B shows where the bytes actually are.
...
**Verified, not asserted**: after the numbers above were committed, `[W3]`, `[W7]` and `[W9]` were re-run against the same code (`--section=W3`, then `--section=W7,W9`) and diffed line by line against `RESULTS.md`. Every structural column reproduced *exactly* — crash row counts and integrity verdicts, the torn-WAL prefix fractions, snapshot cadences and byte counts (8,260 / 41,647 / 83,495 B), fold amplification (39.1 / 40.9 / 45.8), the re-fold row count (106,561), bytes per row (177.3 with the index, 169.7 without), redelivery row counts, the dropped-conflict counter (200 of 200), and both fold digests (`afd706883b5e6260…`, `6f7533725b1f68edf318e2f1…`). Only timing columns differed, and only inside bands already quoted here: reopen 0.1–0.5 ms, cold fold 200–212 ms, boot-with-snapshot 15.5–16.6 ms, `[W9]`'s index cost 1.2–13.6%. That is the artifact's contract in one sentence: **the committed code still produces the committed structure**, and the numbers that move are the ones this appendix says move.

**Timings move**, and this ADR quotes bands where the band matters: six passes of `[W9]`'s index cost gave 1.2 / 3.0 / 11.8 / 13.6 / 15.6 / 25.1%. The low end is a cache artifact, not evidence the index is free — in that pass *both* sides slowed (`without` read 105,108 rows/s against 116,982–121,364 in the other five), so the earlier claim that the `without` side was stable is wrong and is corrected here: the ratio moves because the pair moves. Four passes of `[W8]`'s chain cost gave 9.8 / 10.7 / 17.3 / 20.1%, and `[W1]`'s identical configurations moved up to ~20% between passes. Where a single number is quoted above it is the committed run's; where the spread is wider than the claim, the band is quoted and the ordering is the finding.
