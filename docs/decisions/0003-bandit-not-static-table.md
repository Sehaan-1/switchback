...
The fairness check ADR-0002 mandated is in the record, not waved away: a table that is *not* latency-blind — the same snapshot plus a static per-ms penalty on each processor's *nominal* p99 (`table_snapshot_lat`, ADR-0002's "static table + latency rule" baseline) — still loses **−1,503 / −4,229**. It knows charlie is slower than alpha on paper; it does not know charlie got 3× slower mid-run, so it keeps routing to charlie through the outage and eats a 14.7% timeout rate against the bandit's 6.4%. The degradation, not the nominal latency, is what the bandit learns and the table cannot.
...
- **#17 (benchmarks).** In addition to ADR-0002's five definitions, the algorithm baselines are now fixed: UCB, ε-greedy (two ε), EXP3, A/B-rebalance, and the static tables (`table_global`, `table_snapshot`, `table_snapshot_lat`). Every reported number carries estimate quality (staleness MAE) next to margin, because the F3 finding — a reward-only report would have hidden the re-discovery gap — is only visible with both.
...
- **Static weight table** (the ticket's own counter-proposal, in its strongest forms). *Global 60/30/10*: −4,858 ¢/1k — wrong for whole BIN classes and blind to time. *Per-BIN-class snapshot, never refreshed*: −1,711 full-run / −4,383 post-drift, with a 15.3% timeout rate post-outage and 9.18-pt staleness. *The same snapshot plus a static latency rule* (ADR-0002's fairness baseline): −1,503 / −4,229 — it prices nominal latency but not the degradation. All three are, in the end, the bandit with learning disabled; the steelman is that they are trivially auditable, and that claim survives — which is why the *export* form is adopted while the *control* form is rejected.
